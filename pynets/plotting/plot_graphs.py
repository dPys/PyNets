...
def plot_conn_mat(conn_matrix, labels, out_path_fig):
    ...
    if rois_num < 100:
        try:
            plot_matrix(conn_matrix, cmap='RdYlBu_r', figure=(10, 10), labels=labels, vmax=z_max*0.5, vmin=z_min*0.5,
                        reorder=True, auto_fit=True, grid=False, colorbar=False)
        except RuntimeWarning:
            print('Connectivity matrix too sparse for plotting...')
    else:
        try:
            plot_matrix(conn_matrix, cmap='RdYlBu_r',figure=(10, 10), vmax=z_max*0.5, vmin=z_min*0.5, auto_fit=True,
