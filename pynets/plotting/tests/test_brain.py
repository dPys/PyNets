...
def test_plot_all_nonet_with_mask(connectivity_data):
    ...
    print(
        "%s%s%s"
        % (
            "plot_all (Masking version) --> finished: ",
            str(np.round(time.time() - start_time, 1)),
            "s",
        )
    )
...
def test_plot_network_clusters(connectivity_data, plot_overlaps):
    ...
    temp_file = tempfile.NamedTemporaryFile(
        mode="w+", prefix="figure", suffix=".png"
    )
...
@pytest.mark.parametrize(
    "prune, node_cmap", [pytest.param(3, 0), pytest.param(False, "Set2")]
)
def test_create_gb_palette(connectivity_data, prune, node_cmap):
    ...
    palette = brain.create_gb_palette(
        conn_matrix,
        color_theme,
        coords,
        labels,
...
def test_plot_all_struct(connectivity_data, gen_mat_data, roi):
    ...
    labels = connectivity_data["labels"]
    coords = connectivity_data["coords"]
    conn_matrix = gen_mat_data(
        m=len(labels), n=len(labels), asfile=False, mat_type="sb"
    )["mat_list"][0]
