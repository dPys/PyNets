...
def test_apply_warp():
    ...
    utils.apply_warp(
        ref, inp, out, warp, xfm=xfm, mask=None, interp=None, sup=False
    )
...
def test_match_target_vox_res():
    ...
    anat_img_file = utils.match_target_vox_res(
        anat_img_file, anat_vox_size, anat_out_dir, remove_orig=False
    )
    ...
    dwi_img_file = utils.match_target_vox_res(
        dwi_img_file, dwi_vox_size, dwi_out_dir, remove_orig=False
    )
...
def test_reorient_dwi():
    ...
    dwi_prep_out, bvecs_out = utils.reorient_dwi(
        dwi_prep_neu, bvecs_orig, out_dir
    )
...
def test_reorient_img():
    ...
    check_LAS_input = np.allclose(
        LAStoRAS_img.affine.astype("int"), orig_RAS_img.affine.astype("int")
    )
    check_RAS_input = np.allclose(
        RAStoRAS_img.affine.astype("int"), orig_RAS_img.affine.astype("int")
    )
    check_both_outputs = np.allclose(
        LAStoRAS_img.affine.astype("int"), RAStoRAS_img.affine.astype("int")
    )
...
def test_orient_reslice():
    ...
    # Diffusion: 2x2x2mm
    dmri_LAS = (
        f"{test_dir}/dmri_LAS/iso_eddy_corrected_data_denoised_LAS." f"nii.gz"
    )
    dmri_RAS = (
        f"{test_dir}/dmri_RAS/iso_eddy_corrected_data_denoised_RAS." f"nii.gz"
    )
    bvecs_LAS = f"{test_dir}/dmri_LAS/bvec.orig.bvec"
    bvecs_RAS = f"{test_dir}/dmri_RAS/bvec.trans.bvec"

    anat_LAStoRAS = utils.orient_reslice(anat_LAS, test_dir, "2mm", bvecs=None)
    anat_RAStoRAS = utils.orient_reslice(anat_RAS, test_dir, "2mm", bvecs=None)
    ...
    anat_check = np.allclose(
        anat_LAStoRAS.affine.astype("int"), anat_RAStoRAS.affine.astype("int")
    )
    dmri_check = np.allclose(
        dmri_LAStoRAS.affine.astype("int"), dmri_RAStoRAS.affine.astype("int")
    )
...
def test_make_median_b0():
    ...
    dwi_file = (
        f"{base_dir}/BIDS/sub-25659/ses-1/dwi/final_preprocessed_" f"dwi.nii.gz"
    )
...
def test_rescale_affine_to_center():
    ...
    out_aff = utils.rescale_affine_to_center(input_affine, voxel_dims=[2, 2, 2])
