...
logger.setLevel(50)
from pynets.fmri.estimation import (
    get_conn_matrix,
...
@pytest.mark.parametrize(
    "n_features", [50, pytest.param(2, marks=pytest.mark.xfail)]
)
def test_get_conn_matrix(conn_model_in, n_features):
    ...
    if "corr" in conn_model_in:
        ...
        assert_array_almost_equal(np.diag(conn_matrix_out), np.ones(n_features))
...
@pytest.mark.parametrize(
    "conf", [True, pytest.param(False, marks=pytest.mark.xfail)]
)
@pytest.mark.parametrize("hpass", [0, 0.028, 0.080])
@pytest.mark.parametrize("mask", [True, False])
@pytest.mark.parametrize(
    "func", [True, pytest.param(False, marks=pytest.mark.xfail)]
)
def test_timseries_extraction(
    fmri_estimation_data, parcellation_data, conf, hpass, mask, func
):
