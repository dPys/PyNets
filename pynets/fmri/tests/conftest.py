...
def generate_random_img(
    shape, length=1, affine=np.eye(4), rand_gen=np.random.RandomState(0)
):
...
def fmri_estimation_data():
    ...
    func_img, mask_img = generate_random_img(
    ...
    conf_df = pd.DataFrame(
        {"Conf1": conf_mat, "Conf2": [np.nan] * len(conf_mat)}
    )
    ...
    func_img2, mask_img2 = generate_random_img(
        shape2, affine=affine2, length=length
    )
...
def parcellation_data():
    ...
    labels = pd.DataFrame({"label": list(map("label_{}".format, range(16)))})[
        "label"
    ].values.tolist()
