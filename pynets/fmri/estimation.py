...
def get_optimal_cov_estimator(
    time_series: np.ndarray, cv: int = 5, max_iter: int = 200
) -> object:
    """
    Estimate the optimal covariance estimator for the input time-series.
    ...
    max_iter : int, optional
        Maximum number of iterations for the iterative covariance estimator.
        Default is 200.

    Returns
    -------
    object
        An object of the iterative covariance estimator.
    ...
    try:
        ...
        while (
            not hasattr(estimator, "covariance_")
            and not hasattr(estimator, "precision_")
            and ix < 3
        ):
            for t in [0.1, 0.01, 0.001, 0.0001]:
                print(f"Tolerance={t}")
                estimator = GraphicalLassoCV(
                    cv=cv, max_iter=max_iter, tol=t, assume_centered=True
                )
                try:
                    estimator.fit(time_series)
                    return estimator
        ...
        try:
            ...
            for i in np.arange(0.8, 0.99, 0.01):
                ...
                for alpha in alphaRange:
                    ...
                    estimator_shrunk = GraphicalLasso(
                        alpha, assume_centered=True
                    )
                    try:
...
def get_conn_matrix(
    ...
    def _fallback_covariance(time_series: np.ndarray) -> np.ndarray:
        ...
        print(
            "Matrix estimation failed with Lasso and shrinkage due to "
        ...
        try:
            ...
            conn_measure = ConnectivityMeasure(
                cov_estimator=covariance.LedoitWolf(
                    store_precision=True, assume_centered=True
                ),
                kind=kind,
            )
            ...
            try:
                ...
                raise ValueError(
                    "All covariance estimators failed to " "converge..."
                )
    ...
    if conn_model in nilearn_kinds:
        ...
        if estimator:
            conn_measure = ConnectivityMeasure(
                cov_estimator=estimator, kind=kind
            )
            try:
                conn_matrix = conn_measure.fit_transform([time_series])[0]
            except (np.linalg.linalg.LinAlgError, FloatingPointError):
        ...
        if conn_model == "QuicGraphicalLasso":
            ...
            try:
                ...
                print(
                    e, "Cannot run QuicGraphLassoEBIC. " "Skggm not installed!"
                )
            ...
            try:
                ...
                print(
                    e, "Cannot run AdaptiveGraphLasso. " "Skggm not installed!"
                )
...
def timeseries_bootstrap(
    tseries: np.ndarray, block_size: int
    ...
    block_mask : array_like
        Boolean mask of the bootstrap blocks.
...
def fill_confound_nans(
    confounds: pd.DataFrame, dir_path: str, drop_thr: float = 0.50
) -> str:
    ...
    confounds_nonan = confounds_nonan.dropna(
        thresh=len(confounds_nonan) * float(drop_thr), axis=1
    )
