...
def functional_connectometry(func_file, ID, atlas_select, network, node_size, roi, thr, uatlas_select, conn_model,
    ...
    map_fields = ['conn_model', 'dir_path', 'conn_matrix', 'node_size', 'smooth', 'dens_thresh', 'network', 'ID',
                  ...
                  'label_names', 'coords', 'c_boot', 'norm', 'binary', 'hpass']
        ...
        if not conn_model_list and (node_size_list or smooth_list or float(c_boot) > 0 or hpass_list):
            ...
            if multi_thr:
                ...
                if user_atlas_list or multi_atlas or flexi_atlas is True or float(k_clustering) > 1:
                    ...
                    join_iters_node = pe.Node(niu.IdentityInterface(fields=map_fields), name='join_iters_node')
                    functional_connectometry_wf.connect([(thr_info_node, join_iters_node, map_connects)])
        elif conn_model_list and (node_size_list and smooth_list and float(c_boot) > 0 and hpass_list) or (node_size_list or smooth_list or float(c_boot) > 0) or hpass_list:
    ...
    if plot_switch is True:
        ...
        if conn_model_list or node_size_list or smooth_list or float(c_boot) > 0 or multi_thr or user_atlas_list or multi_atlas or float(k_clustering) > 1 or flexi_atlas is True or hpass_list:
    ...
    if anat_file and (parc is True or float(k_clustering) > 0):
        ...
        functional_connectometry_wf.connect([
            ...
            (node_gen_node, register_atlas_node, [('atlas_select', 'atlas_select'), ('uatlas_select',
                                                                                     'uatlas_select')]),
        ...
        if not conn_model_list and not multi_directget and (node_size_list and parc is False):
            ...
            if multi_thr:
                ...
                if user_atlas_list or multi_atlas or flexi_atlas is True:
                    # print('Multiple atlases...')
                    structural_connectometry_wf.connect([(thr_info_node, join_iters_node_thr, map_connects)])
                    structural_connectometry_wf.connect([(join_iters_node_thr, join_iters_node_prep_spheres,
                                                          map_connects)])
                    structural_connectometry_wf.connect([(join_iters_node_prep_spheres, join_iters_node_atlas,
                                                          map_connects)])
                    structural_connectometry_wf.connect([(join_iters_node_atlas, join_iters_node, map_connects)])
                else:
                    # print('Single atlas...')
