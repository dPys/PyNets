...
def disparity_filter_alpha_cut(G, weight="weight", alpha_t=0.4, cut_mode="or"):
    ...
    cut_mode : str
            ...
            to filter out edges that do not pass the threshold value, combining
             the alpha_in and alpha_out attributes resulting from the
             disparity_filter function. Default is 'or'.
    ...
    if nx.is_directed(G):  # Directed case:
        ...
        for u, v, w in G.edges(data=True):
            try:
                alpha_in = w["alpha_in"]
            except KeyError:  # there is no alpha_in, so we assign 1. It will
                # never pass the cut
                alpha_in = 1
            try:
                alpha_out = w["alpha_out"]
            except KeyError:  # there is no alpha_out, so we assign 1. It will
                # never pass the cut
                alpha_out = 1
        ...
        for u, v, w in G.edges(data=True):
            try:
                alpha = w["alpha"]
            except KeyError:  # there is no alpha, so we assign 1. It will
                # never pass the cut
                alpha = 1
...
def local_thresholding_prop(conn_matrix, thr):
    ...
    References
    ----------
    .. [1] Alexander-Bloch, A. F., Gogtay, N., Meunier, D., Birn, R.,
      Clasen, L., Lalonde, F., … Bullmore, E. T. (2010). Disrupted modularity
      and local connectivity of brain functional networks in childhood-onset
    ...
    if len_edges > edgenum:
        print(
            ...
            f"edges, select more edges. Local Threshold will be applied by "
            f"just retaining the Minimum Spanning Tree")
...
def perform_thresholding(
    ...
    if min_span_tree is True:
        print(
            "Using local thresholding option with the Minimum Spanning Tree "
            "(MST)...\n"
        )
        ...
        # G2 = nx.Graph([(u, v, d) for u, v, d in G1.edges(data=True) if
        # d['alpha'] < thr])
        ...
        # print(f'Backbone graph: nodes = {G2.number_of_nodes()},
...
def thresh_struct(
    ...
    thr : float
        A value, between 0 and 1, to threshold the graph using any variety of
        method triggered through other options.
    conn_matrix : array
        Adjacency matrix stored as an m x n array of nodes and edges.
    conn_model : str
       Connectivity estimation model (e.g. corr for correlation, cov for
