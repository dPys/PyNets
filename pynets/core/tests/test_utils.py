...
def test_save_coords_and_labels_to_json(connectivity_data):
    ...
    nodes_path = utils.save_coords_and_labels_to_json(
        coords, labels, dir_path, subnet, indices
    )
...
def test_save_ts_to_file():
    ...
    out_path_ts = utils.save_ts_to_file(
        roi,
        subnet,
        ID,
        dir_path,
...
def test_collect_pandas_df(plot_switch, embed):
    ...
    net_mets_csv_list = [
        i
        for i in glob.glob(f"{base_dir}/topology/*.csv")
        if "_neat.csv" not in i
    ]
    out = utils.collect_pandas_df(
        subnet,
        ID,
        net_mets_csv_list,
        plot_switch,
...
def test_create_unthr_path(node_size, hpass, smooth, parc):
    ...
    unthr_path_func = utils.create_raw_path_func(
        ID,
        subnet,
        conn_model,
        roi,
    ...
    unthr_path_diff = utils.create_raw_path_diff(
        ID,
        subnet,
        conn_model,
        roi,
...
def test_pass_meta_ins_multi(gen_mat_data, random_mni_roi_data):
    ...
    est_path_func = gen_mat_data(binary=True, mat_type="er")["mat_file_list"][0]
...
def test_collectpandasjoin():
    ...
    net_mets_csv = (
        f"{base_dir}/miscellaneous/002_rsn-Default_nodetype-parc_"
...
def test_build_mp_dict(gen_mat_data, modality):
    ...
    if modality == "func":
        file_orig = gen_mat_data(binary=True, mat_type="er")["mat_file_list"][0]
        file_renamed = (
            f"{os.path.dirname(file_orig)}/graph_sub-002_modality-"
        ...
        file_renamed = (
            f"{os.path.dirname(file_orig)}/0025427_"
    ...
    metaparam_dict = {}
    metaparam_dict, metaparams = build_mp_dict(
        file_renamed, modality, metaparam_dict, gen_metaparams
    )
