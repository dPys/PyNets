...
def create_feature_space(df, grid_param, subject_dict, ses, modality, alg,
    ...
    for ID in df['participant_id']:
        ...
        if alg == 'OMNI' or alg == 'ASE':
            df_lps = flatten_latent_positions('triple', subject_dict, ID, ses,
                                              modality, grid_param, alg)
        else:
    ...
    for modality in modalities:
        ...
        for alg in embedding_types:
            for ses_name in sessions:
                ...
                #
                # for d in outs:
                #     subject_dict = dict(mergedicts(subject_dict, d))
                # del outs
    ...
    if grid_param in ml_dfs[modality][alg].keys():
        ...
        if df_all is None:
            ...
            try:
                ...
                if df_all.empty or len(df_all.columns) == 1:
                    df_all = pd.Series()
            except:
                df_all = pd.Series()
    else:
...
def concatenate_frames(out_dir, modality, alg, files_):
    import pandas as pd
...
def create_wf(base_dir, dict_file_path, modality_grids, drop_cols,
    ...
    concatenate_frames_node = pe.Node(
        niu.Function(
            input_names=["out_dir", "files_", "modality", "alg"],
...
def main():
    ...
    # out = build_predict_workflow(base_dir, dict_file_path, modality_grids,
    #                              drop_cols, target_vars, embedding_types,
    #                              target_modality)

    args = {}
    args['base_dir'] = base_dir
    args['dict_file_path'] = dict_file_path
