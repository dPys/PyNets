...
def test_discr_stat():
                ...
                for n_features in [100, 1000]:
                    ...
                    for n_samples in [100, 1000]:
                        ...
                        disc = benchmarking.discr_stat(
                            X, Y, dissimilarity=dissimilarity
                        )[0]
                        mega_dict[remove_isolates][
                            f"n_informative_{n_informative}"
                        ][f"n_redundant_{n_redundant}"][
    ...
    d1 = np.array(
        list(
            NestedDictValues(get_recursively(mega_dict, "n_informative_49")[0])
        )
    )
    d2 = np.array(
        list(
            NestedDictValues(get_recursively(mega_dict, "n_informative_25")[0])
        )
    )
    assert ttost_paired(d1, d2, 0, 1)
    assert ttost_paired(d1, d2, 0, 1)[0] > 0.5
    ...
    d1 = np.array(
        list(NestedDictValues(get_recursively(mega_dict, "n_features_100")[0]))
    )
    d2 = np.array(
...
@pytest.mark.parametrize("n_features", [100, 1000])
@pytest.mark.parametrize("n_samples", [100, 1000])
def test_discr_rdf(n_informative, n_redundant, n_features, n_samples):
    X, Y = make_classification(
        n_samples=n_samples,
        n_features=n_features,
        n_informative=n_informative,
    ...
    assert (
        np.sum(rdfs >= 0) <= rdfs.shape[0] * rdfs.shape[1]
        and np.sum(rdfs <= 1) <= rdfs.shape[0] * rdfs.shape[1]
    )
