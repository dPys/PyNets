...
@pytest.mark.parametrize(
    "granularity", [10, 100, pytest.param(1, marks=pytest.mark.xfail)]
)
@pytest.mark.parametrize(
    "n_graphs",
    [
...
def test_omni(gen_mat_data, granularity, n_graphs, modality):
    ...
    assert (
        ...
        and output_path.endswith(".npy")
        and emb_mat.shape[0] == granularity
        and emb_mat.shape[1] == 1
    )


@pytest.mark.parametrize(
    "granularity", [100, pytest.param(1, marks=pytest.mark.xfail)]
)
@pytest.mark.parametrize(
    "n_graphs",
    [
...
def test_MASE(gen_mat_data, granularity, n_graphs):
    ...
    atlas = "tmp"
    output_file = spectral._mase_embed(
        [pop_array_func[0], pop_array_dwi[0]], atlas, pop_file_list_func[0]
    )
    ...
    assert (
        ...
        and output_file.endswith(".npy")
        and emb_mat.shape[0] == granularity
        and emb_mat.shape[1] == 2
    )

    output_path = spectral.build_masetome(
        [list(itertools.product([pop_file_list_func, pop_file_list_dwi]))[0][0]]
    )[0]
    emb_mat = np.load(output_path)
    assert (
        Path(output_path).is_file()
...
def test_ASE(gen_mat_data, granularity, n_graphs, modality):
    ...
    output_file = spectral._ase_embed(
        mat_dat["mat_list"][0], atlas, mat_dat["mat_file_list"][0]
    )
...
@pytest.mark.parametrize(
    "n_graphs", [2, 1, pytest.param(0, marks=pytest.mark.xfail)]
)
@pytest.mark.parametrize("modality", ["func", "dwi"])
