...
def test_average_shortest_path_length_fast(gen_mat_data, weight):
    ...
    print(
        "%s%s%s"
        % (
            "test_average_shortest_path_length_fast --> finished: ",
            np.round(time.time() - start_time, 1),
            "s",
        )
    )
...
def test_average_local_efficiency(gen_mat_data, engine):
    ...
    print(
        "%s%s%s"
        % (
            "test_average_local_efficiency --> finished: ",
            np.round(time.time() - start_time, 1),
            "s",
        )
    )
...
@pytest.mark.parametrize("method", ["betweenness", "coreness", "eigenvector"])
@pytest.mark.parametrize("engine", ["GT", "NX"])
def test_most_important(gen_mat_data, method, engine):
    ...
    start_time = time.time()
    Gt, pruned_nodes = algorithms.most_important(
        G, method=method, engine=engine
    )
    print(
        "%s%s%s"
        % (
            "test_most_important --> finished: ",
            str(np.round(time.time() - start_time, 1)),
            "s",
        )
    )
...
def test_smallworldness(gen_mat_data, approach, reference):
    ...
    sigma = algorithms.smallworldness(
        G, niter=5, nrand=5, approach=approach, reference=reference, engine="nx"
    )
...
@pytest.mark.parametrize(
    "metric",
    ["participation", "diversity", "local_efficiency", "comm_centrality"],
)
@pytest.mark.parametrize("engine", ["GT", "NX"])
def test_get_metrics(gen_mat_data, metric, engine):
    ...
    in_mat = gen_mat_data(asfile=False, mat_type="sb", binary=binary)[
        "mat_list"
    ][0]
    ...
    if metric == "participation":
        ...
        assert (
            len(metric_list_names)
            == len(algorithms.participation_coef(in_mat, ci)) + 1
        )
        assert (
            len(net_met_val_list_final)
            == len(algorithms.participation_coef(in_mat, ci)) + 1
        )
    elif metric == "diversity":
        metric_list_names, net_met_val_list_final = algorithms.get_diversity(
            in_mat, ci, metric_list_names, net_met_val_list_final
        )
        assert (
            len(metric_list_names)
        ...
        assert (
            len(metric_list_names)
            == len(nx.algorithms.communicability_betweenness_centrality(G)) + 1
        )
        assert (
            len(net_met_val_list_final)
            == len(nx.algorithms.communicability_betweenness_centrality(G)) + 1
