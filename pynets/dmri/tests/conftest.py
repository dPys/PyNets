...
def dmri_estimation_data():
    ...
    b0_mask, mask = median_otsu(
        np.asarray(dwi_data, dtype=np.float32)[..., b0_ixs[0]],
        median_radius=4,
        numpass=2,
    )
    ...
    mask_img_small = nib.Nifti1Image(
        mask[40:50, 75:85, 40:50].astype("int32"), affine=mask_img.affine
    )
    ...
    gtab_file_tmp = tempfile.NamedTemporaryFile(
    ...
    dwi_data_small = dwi_data_small[40:50, 75:85, 40:50]
...
def tractography_estimation_data(dmri_estimation_data):
    path_tmp = tempfile.NamedTemporaryFile(
        mode="w+", suffix=".trk", delete=False
    )
    ...
    mask_img = intersect_masks(
        [
            nib.Nifti1Image(
                np.asarray(wm_img.dataobj).astype("bool").astype("int"),
                affine=wm_img.affine,
            ),
            nib.Nifti1Image(
                np.asarray(B0_mask_img.dataobj).astype("bool").astype("int"),
    ...
    mask_file = fname_presuffix(
        dmri_estimation_data["B0_mask"], suffix="tracking_mask", use_ext=True
    )
    ...
    sft = StatefulTractogram(
        streamlines, B0_mask_img, origin=Origin.NIFTI, space=Space.VOXMM
    )
