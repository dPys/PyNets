...
def test_create_anisopowermap(dmri_estimation_data):
    ...
    assert os.path.isfile(
        create_anisopowermap(gtab_file, dwi_file, B0_mask_file)[0]
    )
...
def test_tens_mod_est(dmri_estimation_data):
    ...
    [mod_odf, model] = tens_mod_est(gtab, dwi_data, B0_mask_file)
...
def test_csa_mod_est(dmri_estimation_data):
    ...
    [csa_mod, model] = csa_mod_est(gtab, dwi_data, B0_mask_file, sh_order=0)
...
def test_csd_mod_est(dmri_estimation_data):
    ...
    [csd_mod, model] = csd_mod_est(
        gtab, dwi_data_small, B0_mask_file_small, sh_order=0
    )
...
def test_streams2graph(
    dmri_estimation_data,
    ...
    atlas_in_dwi = fname_presuffix(ap_path, suffix="atlas_in_dwi", use_ext=True)
    resample_to_img(
