...
def test_create_density_map():
    ...
    streamlines_trk = (
        f"{base_dir}/miscellaneous/streamlines_model-csd_"
    ...
    [dir_path, dm_path] = track.create_density_map(
        dwi_img,
        dir_path,
        streams_final_filt_final,
        conn_model,
...
def test_prep_tissues(tiss_class):
    ...
    tiss_classifier = track.prep_tissues(
        nib.load(t1w_mask),
        nib.load(gm_in_dwi),
...
@pytest.mark.parametrize(
    "target_samples", [300, pytest.param(0, marks=pytest.mark.xfail)]
)
def test_track_ensemble(traversal, target_samples):
    ...
    dwi_file = (
        f"{base_dir}/003/test_out/003/dwi/sub-003_dwi_reor-RAS_res-"
        f"2mm.nii.gz"
    )
    atlas_data_wm_gm_int = (
        f"{dir_path}/whole_brain_cluster_labels_PCA200_"
        f"dwi_track_wmgm_int.nii.gz"
    )
    labels_im_file = (
        f"{dir_path}/whole_brain_cluster_labels_PCA200_dwi_" f"track.nii.gz"
    )
    conn_model = "csa"
    tiss_class = "wb"
    min_length = 2
...
def test_track_ensemble_particle():
    ...
    gtab = gradient_table(bvals, bvecs)
    dwi_file = (
        f"{base_dir}/003/test_out/003/dwi/sub-003_dwi_reor-RAS"
        f"_res-2mm.nii.gz"
    )
    atlas_data_wm_gm_int = (
        f"{dir_path}/whole_brain_cluster_labels_PCA200"
        f"_dwi_track_wmgm_int.nii.gz"
    )
    labels_im_file = (
        f"{dir_path}/whole_brain_cluster_labels_PCA200_dwi" f"_track.nii.gz"
    )
    conn_model = "csd"
    tiss_class = "cmc"
    min_length = 2
