...
def build_workflow(args, retval):
    ...
    if environ.get("FSLDIR") is None:
        print(
            "EnvironmentError: FSLDIR not found! "
            "Be sure that this "
        ...
        fsl_version = check_output(
            'flirt -version | cut -f3 -d" "', shell=True
        ).strip()
        fsldir = os.environ["FSLDIR"]

        if fsl_version and os.path.isdir(fsldir):
            print(
                f"{Fore.MAGENTA}FSL {fsl_version.decode()} detected: "
                f"FSLDIR={fsldir}"
            )
        else:
            print(
                "Is your FSL installation corrupted? Check permissions and"
        ...
        if len(graph) > 1:
            ...
            if os.path.isdir(graph):
                ...
                if isinstance(ID, list):
                    ...
                    multi_subject_multigraph = None
                    multi_graph = [str(g) for g in graph_iter if ID in str(g)]
                graph = None
                multi_subject_graph = None
            else:
                if "," in graph:
        ...
        if len(include_str_matches) > 0:
            if multi_graph is not None:
                multi_graph = [
                    ...
                    if any(i for j in include_str_matches if str(j) in i)
                ]
            if multi_subject_graph is not None:
                multi_subject_graph = [
                    i
                    for i in multi_subject_graph
                    if any(i for j in include_str_matches if str(j) in i)
                ]
            if multi_subject_multigraph is not None:
                multi_subject_multigraph = [
                    i
                    for i in multi_subject_multigraph
                    if any(i for j in include_str_matches if str(j) in i)
                ]
    else:
        multi_graph = None
    ...
    if (
        ...
        and not graph
        and not multi_graph
        and not multi_subject_graph
        and not multi_subject_multigraph
    ):
        print(
            "\nERROR: You must include a subject ID in a command " "line call."
        )
        retval["return_code"] = 1
        return retval
    ...
    if resources == "auto":
        ...
        procmem = [int(psutil.cpu_count()), [vmem if vmem > 8 else int(8)][0]]
    else:
    ...
    if atlas_ins is not None:
        ...
        for atl in atlas_ins:
            if (
                atl in nilearn_parc_atlases
                or atl in nilearn_coord_atlases
                or atl in nilearn_prob_atlases
                or atl in local_atlases
            ):
                atlas.append(atl)
            elif "/" in atl:
    ...
    if (
        (min_thr is not None)
        and (max_thr is not None)
        and (step_thr is not None)
    ):
        multi_thr = True
    elif (
    ...
    if mask:
        if isinstance(mask, list) and func_file_list and dwi_file_list:
            if len(mask) != len(func_file_list) and len(mask) != len(
                dwi_file_list
            ):
                print(
    ...
    if (
        ...
        print(
            f"{Fore.YELLOW}Running workflow for single subject: "
            f"{Fore.BLUE}{str(ID)}"
        )

    print(
        f"{Fore.GREEN}Population template: "
    ...
    if (
        ...
        if subnet is not None:
            ...
            print(f"{Fore.BLUE}{str(', '.join(str(n) for n in multi_nets))}")
        else:
        ...
        if conn_model_list:
            ...
            print(
                f"{Fore.GREEN}Using connectivity model: "
                f"{Fore.BLUE}{conn_model}..."
            )

    elif (
        graph or multi_graph or multi_subject_graph or multi_subject_multigraph
    ):
    ...
    if func_file or func_file_list:
        ...
        if k_clustering == 1:
            ...
            print(
                f"{Fore.GREEN}Clustering within mask at a single"
                f" resolution..."
            )
        elif k_clustering == 2:
            print(
                f"{Fore.GREEN}Clustering within mask at multiple"
                f" resolutions:"
            )
            if func_file_list:
            ...
            if func_file_list:
                for _clust_type in clust_type_list:
                    for _clust_mask in clust_mask_list:
                        for _k in k_list:
                            cl_mask_name = op.basename(_clust_mask).split(
                                ".nii"
                            )[0]
                            atlas_clust = f"{cl_mask_name}_{_clust_type}_k{_k}"
                            print(f"{Fore.BLUE}{atlas_clust}")
    ...
    if dwi_file or dwi_file_list:
        ...
        if traversal:
            print(
                f"{Fore.GREEN}Using {Fore.BLUE}{traversal} "
        ...
        if min_length:
            print(
                f"{Fore.GREEN}Using {Fore.BLUE}{min_length}mm{Fore.GREEN} "
        ...
        if error_margin:
            ...
            print(
                f"{Fore.GREEN}Using {Fore.BLUE}{error_margin}"
            ...
            for em in error_margin_list:
                if float(roi_neighborhood_tol) <= float(em):
                    ...
                    return retval
            print(
                f"{Fore.GREEN}Iterating ROI-streamline intersection "
                f"tolerance:"
            )
        ...
        if target_samples:
            print(
                f"{Fore.GREEN}Using {Fore.BLUE}{target_samples} "
                f"{Fore.GREEN}streamline samples..."
            )
        print(
            f"{Fore.GREEN}Using {Fore.BLUE}{track_type} "
            f"{Fore.GREEN}tracking with {Fore.BLUE}{tiss_class} "
            f"{Fore.GREEN}tissue classification..."
        )
        print(
            f"{Fore.GREEN}Ensemble tractography step sizes: "
    ...
    if (dwi_file or dwi_file_list) and not (func_file or func_file_list):
        ...
        if dwi_file_list:
            for (_dwi_file, _fbval, _fbvec, _anat_file) in list(
                ...
                print(
                    f"{Fore.GREEN}Diffusion-Weighted Image:{Fore.BLUE}\n "
                    f"{_dwi_file}"
                )
            ...
            print(
                f"{Fore.GREEN}Diffusion-Weighted Image:\n "
        ...
        if conf_list:
            for _conf in conf_list:
                print(
                    f"{Fore.GREEN}BOLD Confound Regressors:\n "
                    f"{Fore.BLUE}{_conf}"
                )
    ...
    if anat_file or anat_file_list:
        if anat_file_list and len(anat_file_list) > 1:
            for anat_file in anat_file_list:
                print(
                    f"{Fore.GREEN}T1-Weighted Image:\n "
    ...
    def init_wf_single_subject(
        ...
        multi_thr : list
            List of multiple thresholds to use for the multi-threshold method.
        multi_atlas : list
            List of atlas names used in the multi-atlas analysis (e.g. ['aal', 'basc']).
        min_thr : int
        ...
        anat_file : str
            File path to anatomical image.
        parc : bool
            Indicates whether to use parcels instead of coordinates as ROI nodes.
        ref_txt : str
            File path to a text file containing a list of reference ROI's to be
            used in the analysis. The file should contain one ROI per line and
            should correspond to the one or more parcellation files used.
        procmem : int
            Memory limit for processing in GB (default is 8GB).
        k : int
            Number of clusters that will be generated.
        clust_mask : str
            File path to a 3D NIFTI file containing a mask, which restricts the
            voxels used in the clustering process.
        k_list : list
            List of numbers of clusters to be generated.
        k_clustering : int
            Number of clusters to be generated.
        user_atlas_list : list
            List of atlas parcellations to be used.
        clust_mask_list : list
            List of 3D NIFTI files containing masks, which restrict the voxels
            used in the clustering.
        prune : int
            Indicates whether and how to prune final graph of disconnected
            nodes/isolates.
        node_radius_list : list
            List of node radii to be used in the case that coordinate-based
            centroids are used as ROI's.
        graph : str
            File path to a graph in .txt, .csv, .json, .npy, .mat, .gpickle,
            .tsv, or .ssv format.
        conn_model_list : str
            List of connectivity estimation models (e.g. corr for correlation,
            cov for covariance, sps for precision covariance, partcorr for
            partial correlation).
        min_span_tree : bool
        ...
        plugin_type : str
            The plugin type to be used for the workflow.
        use_parcel_naming : bool
            Indicates whether to use the parcel naming convention for the
            atlas.
        multi_graph : bool
            Indicates whether to use multiple graphs in the workflow or not.
        smooth : int
            Smoothing width (mm fwhm) to apply to time-series when extracting
            signal from ROI's.
        smooth_list : list
            List of smoothing widths (mm fwhm) to apply to time-series when
            extracting signal from ROI's.
        disp_filt : bool
        ...
        clust_type : str
            ...
            'complete', 'average').
        clust_type_list : list
            List of clustering types to be performed (e.g. ['ward', 'kmeans']).
        mask : str
        ...
        binary : bool
            ...
            unweighted graph.
        fbval : str
            File path to a text file containing the b-values of the DWI data.
        fbvec : str
            File path to a file containing the gradient directions for the
            diffusion data.
        curv_thr_list : list
        ...
        embed : bool
            Indicates whether to embed the resulting graph in a higher-dimensional
            space.
        multi_traversal : bool
            Indicates whether to perform multiple traversals of the graph.
        multimodal : bool
        ...
        hpass_list : list
            List of high-pass filter values (Hz) to apply to node-extracted
            time-series.
        vox_size : str
            Voxel size (mm) of input data.
        multiplex : bool
            Indicates whether multiplex data has been specified.
        waymask : str
            File path to a 3D NIFTI file containing a waymask, which restricts
            the voxels used in the analysis.
        local_corr : str
        ...
        min_length_list : list
            List of minimum fiber length thresholds in mm to restrict tracking.
        error_margin_list : list
            List of Euclidean margin of error for classifying a streamline as
            a connection to an ROI. Default is 2 voxels.
        signal : str
            The name of a valid function used to reduce the time-series region
            extraction.
        signal_list : list
            List of names of valid functions used to reduce the time-series
            region extraction (e.g. 'mean', 'median', 'max', 'min', 'std', 'sum').
        outdir : str
            Path to pynets derivatives output directory.
        """
        import warnings
        ...
        wf = pe.Workflow(name=f"{ID}_{strftime('%Y%m%d_%H%M%S')}")
        ...
        if func_file:
            wf_selected = "fmri"
            for node_name in (
                wf.get_node(meta_wf.name)
                .get_node(wf_selected)
                .list_node_names()
            ):
        ...
        combine_pandas_dfs_node = pe.Node(
            interface=CombineOutputs(), name="CombineOutputs"
        )
    ...
    def wf_multi_subject(
        ...
        dwi_file_list : list
            List of file paths to 4D Nifti1Images containing diffusion data.
        mask_list : list
            List of file paths to Nifti1Images containing brain masks.
        fbvec_list : list
            List of file paths to b-vector data.
        fbval_list : list
            List of file paths to Nifti1Images containing b-values.
        conf_list : list
            List of file paths to Nifti1Images containing confounds.
        anat_file_list : list
            List of file paths to Nifti1Images containing anatomical T1w data.
        atlas : str
            Name of atlas parcellation used.
        network : str
            Resting-state network based on Yeo-7 and Yeo-17 naming
        ...
        thr : float
            ...
            of methods triggered through other options.
        parcellation : str
            File path to a parcellation Nifti1Image file.
        multi_nets : list
            List of Yeo RSN's specified in workflow(s).
        conn_model : str
            Connectivity estimation model (e.g. corr for correlation, cov for
        ...
        dwi_file : str
            File path to diffusion weighted image.
        multi_thr : list
            List of thresholds to use for multi-thresholding.
        multi_atlas : list
            List of atlases used.
        min_thr : float
             Minimum value, between 0 and 1, for the graph thresholding.
        max_thr : float
            Maximum value, between 0 and 1, for the graph thresholding.
        step_thr : float
            A value, between 0 and 1 (and less than max_thr - min_thr), to threshold
            the graph iteratively with multi-thresholding.
        anat_file : str
            File path to anatomical image.
        parc : bool
            Indicates whether to use parcels instead of coordinates as ROI nodes.
        ref_txt : str
            File path to a text file containing a list of parcel labels corresponding to the
            indices of the parcels in the provided parcellation.
        procmem : str
            Memory to be used for the processing.
        k : int
            Number of clusters that will be generated.
        clust_mask : str
            File path to a 3d Nifti1Image file containing a mask, which restricts the
            voxels used in the clustering.
        k_list : list
            List of numbers of clusters to be generated.
        k_clustering : str
            Type of clustering to use (e.g. kmeans, kmedoids, kspectral, kward).
        user_atlas_list : list
            List of user-specified atlas files.
        clust_mask_list : list
            List of file paths to 3d Nifti1Image files containing masks, which restrict
            the voxels used in the clustering. The number of masks in the list
            must match the number of clusters in the list k_list or k_clustering.
        prune : int
            Indicates whether and how to prune final graph of disconnected
            nodes/isolates.
        node_radius_list : list
            List of node radii to be used in the case that coordinates are used
        conn_model_list : list
            List of connectivity estimation models.
        min_span_tree : bool
             Indicates whether local thresholding from the Minimum Spanning
             Tree should be used
        verbose : bool
            Indicates whether to print out verbose output.
        plugin_type : str
            The plugin type to be used for running the workflow.
        use_parcel_naming : bool
            Indicates whether to use the parcel naming convention.
        multi_subject_graph : list
            List of file paths to graphs of the multiple subjects.
        multi_subject_multigraph : list
            List of file paths to the multigraphs of the
            multiple subjects.
        smooth : int
            Smoothing width (mm fwhm) to apply to time-series when extracting
            signal from ROI's
        ...
        clust_type : str
            ...
            'complete', 'average')
        clust_type_list : list
            List of clustering methods to be performed.
        mask : str
             File path to a 3D NIFTI file containing a mask, which restricts
             the voxels used in the analysis
        norm : int
        ...
        binary : bool
            ...
            unweighted graph
        fbval : str
            File path to a text file containing the b-values.
        fbvec : str
            File path to a file containing b-vectors.
        curv_thr_list : list
            List of integer curvature thresholds used to perform ensemble
            tracking
        step_list : list
        ...
        traversal : str
            ...
            prob (probabilistic)
        tiss_class : str
            Tissue classification method for tractography.
        runtime_dict : dict
            Dictionary of runtime parameters.
        execution_dict : dict
            Nipype workflow global settings
        embed : bool
            Indicates whether to embed the graph in a higher-dimensional
            space.
        multi_traversal : list
            List of statistical approaches to tracking
        multimodal : bool
            Indicates whether multiple modalities of input data have been
            specified
        hpass : bool
            High-pass filter values (Hz) to apply to node-extracted time-series
        hpass_list : list
            List of high-pass filter values (Hz) to apply to node-extracted time-series.
        vox_size : str
            Voxel size (mm) of the input data.
        multiplex : bool
            Indicates whether multiple modalities of input data have been specified.
        waymask : str
            File path to a 3d Nifti1Image file containing a mask, which restricts the
            tractography to a target blocking mask of interest.
        local_corr : str
             Type of local connectivity to use as the basis for clustering
             methods. Options are tcorr or scorr. Default is tcorr
        min_length_list : list
            List of minimum fiber length thresholds in mm to restrict tracking.
        error_margin_list : list
            List of Euclidean margins of error for classifying a streamline as a
            connection to an ROI.
        signal : str
            The name of a valid function used to reduce the time-series region
            extraction
        signal_list : list
            List of names of valid functions used to reduce the time-series region
            extraction.
        outdir : str
            Path to pynets derivatives output directory.
    ...
    if (
        ...
        os.makedirs(
            f"{work_dir}/wf_multi_subject_{'_'.join(ID)}", exist_ok=True
        )
        ...
        if verbose is True:
            ...
            if os.path.isfile(callback_log_path):
                ...
                print(
                    f"Cannot plot resource usage. {callback_log_path} not "
                    f"found..."
                )
        ...
        for dir in dir_list:
            if "func" in dir:
                ...
                for file_ in [
                    i for i in glob.glob(f"{dir}/func/*") if os.path.isfile(i)
                ]:
            ...
            if "dwi" in dir:
                ...
                for file_ in [
                    i for i in glob.glob(f"{dir}/dwi/*") if os.path.isfile(i)
                ]:
                    if ("reor-RAS" in file_) or ("res-" in file_):
                        try:
                            os.remove(file_)
    ...
    # Single-subject workflow generator
        ...
        try:
            subj_dir = (
                f"{outdir}/sub-{ID.split('_')[0]}/" f"ses-{ID.split('_')[1]}"
            )
        except BaseException:
        ...
        if func_file:
            for file_ in [
                i for i in glob.glob(f"{subj_dir}/func/*") if os.path.isfile(i)
            ] + [
                i
                for i in glob.glob(f"{subj_dir}/func/*/*")
                if os.path.isfile(i)
            ]:
                if ("reor-RAS" in file_) or ("res-" in file_):
                    try:
        ...
        if dwi_file:
            ...
            if remove_recons is True:
                for file_ in [
                    i
                    for i in glob.glob(
                        f"{subj_dir}/dwi/*/*/" f"tractography/*.hdf5"
                    )
                    if os.path.isfile(i)
                ]:
                    try:
            ...
            if remove_trk is True:
                for file_ in [
                    i
                    for i in glob.glob(
                        f"{subj_dir}/dwi/*/*/" f"tractography/*.trk"
                    )
                    if os.path.isfile(i)
                ]:
                    try:
    ...
    print(
        "Execution Time: ",
        str(timedelta(seconds=timeit.default_timer() - start_time)),
    )
    print("-------------------------------")
    return retval
...
if __name__ == "__main__":
    ...
    warnings.filterwarnings("ignore")
    __spec__ = (
        "ModuleSpec(name='builtins', loader=<class '_frozen"
